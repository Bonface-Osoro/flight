...
d1 <- read.csv(file.path(folder, '..', 'validation', 
                         'jan_model_on_may_images_metrics.csv'))

d2 <- read.csv(file.path(folder, '..', 'validation', 
                         'lwir_model_on_rgb_images_metrics.csv'))

d3 <- read.csv(file.path(folder, '..', 'validation', 
                         'may_model_on_jan_images_metrics.csv'))

d4 <- read.csv(file.path(folder, '..', 'validation', 
                         'rgb_model_on_lwir_images_metrics.csv'))
...
model_comparion_average <- ggplot(df4, aes(x = model, y = mean, fill = model)) +
  ...
  geom_errorbar(aes(ymin = pmax(0, mean - 0.5*sd), ymax = mean + 0.5*sd),
                width = 0.2, color = "red", size = 0.2) +
  geom_text(aes(label = sprintf("%.1f", mean)), size = 0.8, vjust = -0.5, 
            hjust = -0.2, angle = 0) +
...
df2$model <- factor(df2$model,
    levels = c('Jan on May Images', 'May on Jan Images', 'RGB on LWIR Images', 'LWIR on RGB Images'),
...
model_class_test <- ggplot(dff, aes(x = model, y = mean, fill = class)) +
  ...
  geom_text(aes(label = formatC(signif(after_stat(y), 4), 
     digits = 3, format = "fg", flag = "#")), color = 'black', size = 1.5, position = 
...
png(path, units = "in", width = 5, height = 3.5, res = 300)
