...
yolo_f1_score <- ggplot(df5, aes(continuous, mean)) + 
  ...
  geom_smooth(aes(linetype = "Smooth"), method = "loess", se = FALSE, 
              color = "blue", size = 0.7) + 
  labs(linetype = NULL, title = "C", x = "Epochs", y = "F1-Score") +
  scale_linetype_manual(values = c("Results" = "solid", "Smooth" = "dotted")) +
...
## Panel Plots ##
#################
metric_plots <- ggarrange(yolo_losses, cls_metrics, yolo_f1_score, ncol = 3, 
       common.legend = FALSE) 

path = file.path(folder, 'figures', 'jan_metrics.png')
png(path, units = "in", width = 8, height = 3., res = 300)
print(metric_plots)
dev.off()
